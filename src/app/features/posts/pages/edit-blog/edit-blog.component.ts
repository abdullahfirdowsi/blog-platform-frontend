...
export class EditBlogComponent implements OnInit, OnDestroy {
  ...
  private initializeEditForm(blog: Blog): void {
    ...
    this.originalTags = blog.tag_ids || [];
    
    // Initialize edit modal values
    this.mainImageUrl = blog.main_image_url || '';
    
    // Fix tags initialization - use tag names if available, otherwise tag_ids
    if (blog.tags && blog.tags.length > 0) {
      // If we have tag objects with names, use the names
      this.selectedTags = blog.tags.map(tag => 
        typeof tag === 'string' ? tag : tag.name || tag._id
      );
    } else if (blog.tag_ids && blog.tag_ids.length > 0) {
      // Fall back to tag_ids if available
  ...
  performSave(): void {
    ...
    this.blogStateService.updateBlog(this.blogId, updateData).subscribe({
      next: (updatedBlog) => {
        ...
        this.originalTags = updatedBlog.tag_ids || [];
  ...
  closeRepublishModal(): void {
    ...
    // Reset tags - restore the tag names as they were originally loaded
    if (this.blog && this.blog.tags && this.blog.tags.length > 0) {
      this.selectedTags = this.blog.tags.map(tag => 
        typeof tag === 'string' ? tag : tag.name || tag._id
      );
    } else {
