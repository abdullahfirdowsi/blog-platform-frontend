...
export class BlogService {
 ...
 getPosts(filters?: PostFilters): Observable<PostsResponse> {
  ...
  return this.getBlogs(blogFilters).pipe(
    map(blogResponse => {
      console.log("Get Blog Method");
      const posts: PostSummary[] = blogResponse.blogs.map(blog => ({
        id: blog._id,
        title: blog.title,
        excerpt: this.generateExcerpt(blog.content), // excerpt generated from `content`
        ...
        author: {
          id: blog.user_id,
          username: 'Unknown', // Backend does not return user details
          profile_picture: undefined
        },
        tags: Array.isArray(blog.tags)
          ? (typeof blog.tags[0] === 'string'
              ? blog.tags as string[]
              : (blog.tags as any[]).map(tag => typeof tag === 'string' ? tag : tag.name))
          : [],
        views: 0, // views not in backend
        likes_count: blog.likes_count || 0,
        comments_count: blog.comments_count || 0,
        is_liked: false, // not returned by backend
